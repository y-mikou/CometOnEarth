...
修正した結果、体裁崩れの可能性がなくなったことを確認するためのもの。

1段落につき初回に登場するもののみを表示する(1つ目のものを修正したら2つ目以降はズレれるため)。
...
表示可能な範囲で、検出モードで検出できる内容を反映して表示する(文字色反転)。
...
- チェックモード(-c)で、複数の問題が検出された場合には、一つだけが表示されます。
- ビューモード(-v)では、`｜《》`や`《《》》`、`゛`などの表示上の文字数が変更されるものは除去されます。
- 環境・フォント・文字の関係で、全角表示されてほしいのに半角として扱われるものがあります。そうした文字はプログラム内の設定箇所に対象の文字を追記してください。
- 絵文字などは表示では1文字なのに内部的に2文字として扱われるものがあります。そうした文字はプログラム内の設定箇所に対象の文字を追記してください。
- お好みの禁則(行頭登場禁止、行末登場禁止)は、プログラム内の設定箇所に対象の文字を追記してください。
- 
